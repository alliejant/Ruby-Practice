...
# BETTER but doesn't pass tests
def lucas_number_improved(nth)
    calculated = [2,1] 
  
    def helper(nth, calculated)
      if !calculated[nth]
        calculated[nth] = helper(nth-1, calculated) + helper(nth-2, calculated)
      end
      calculated[nth]
    end
  
    helper(nth, calculated)
  end
...
def sum_array(array, index = 0, total = 0)
    return total if index == array.length
    total += array[index]
    sum_array(array, index + 1, total)
end
...
def reverse_string(str, index = str.length-1, reversed = "")
    return reversed if index < 0
    reversed += str[index]
    reverse_string(str, index - 1, reversed)
end
